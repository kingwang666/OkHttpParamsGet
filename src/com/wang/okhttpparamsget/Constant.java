package com.wang.okhttpparamsget;

public class Constant {

    //Map<String, String>
    public static final int TYPE_MAP_STRING = 1;
    //Map<String, Object>
    public static final int TYPE_MAP_OBJECT = 2;
    //Map<String, RequestBody>
    public static final int TYPE_MAP_BODY = 3;
    //List<MultipartBody.Part>
    public static final int TYPE_LIST_PART = 4;
    //MultipartBody.Builder
    public static final int TYPE_BODY_BUILDER = 5;

    public static final String VALUE_NULL = "okhttpparamsget_value_null";

    public static final String ANDROIDX = "okhttpparamsget_android_x";

    public static final String NULLABLE = "okhttpparamsget_null_able";

}
